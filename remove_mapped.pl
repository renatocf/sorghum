...
#######################################################################
# Program:    remove_mapped.pl                                        #
# mantainer:  Renato Cordeiro Ferreira                                #
# usage:      Gets a .gtf file, a .map (generated by create_map.pl)   #
#             and a predictor name. Prints in the stdin the cleant    #
#             .gtf file without the lines within the mapped region.   #
...
open(MASK, "<", $mask_file);
<MASK>; # Discard the first entry

# Put the other maskings in the variable
while(my $one_masking = <MASK>) { 
    push (@maskings, $one_masking); 
}
close(MASK);

# Sort the masked and genes lists
